...
def bright(img):
    ...
    # 增加图像亮度
    v1 = np.clip(cv2.add(1*v,-35),0,255)  #-35
...
def augmentation(rgb, mask, hcoords, height, width, split):
    if split == 'train':
        ...
        img_transforms = transforms.Compose([
            transforms.ColorJitter(0.1, 0.1, 0.05, 0.05),    #随机的颜色抖动，改变亮度[0.9,1.1]，对比度等，原文带的，但是好像没有用到
            #transforms.ColorJitter(0.0, 0.1, 0.05, 0.05),
            transforms.ToTensor(),
