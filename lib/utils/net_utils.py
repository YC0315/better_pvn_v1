...
def load_model(net, optim, scheduler, recorder, model_dir, resume=True, epoch=-1):
    ...
    scheduler.load_state_dict(pretrained_model['scheduler'])   # 加载学习率从断点处的学习率接着训练
...
def save_model(net, optim, scheduler, recorder, epoch, model_dir):  
    ...
    torch.save({                          # 只保存训练好的权重，将网络训练过程中的网络的权重，优化器的权重保存，学习率的权重、记录以及epoch 保存，便于继续训练恢复
        'net': net.state_dict(),
        'optim': optim.state_dict(),
        'scheduler': scheduler.state_dict(),   # scheduler是根据last_step来恢复学习率
