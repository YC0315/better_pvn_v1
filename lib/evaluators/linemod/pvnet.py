...
class Evaluator:
    ...
    def projection_2d(self, pose_pred, pose_targets, K, icp=False, threshold=5):
        #print("pose_pred",pose_pred[:, :3])
        #print("pose_targets",pose_targets[:, :3])
        #print(np.linalg.det(pose_targets[:, :3]))
        #print(np.linalg.det(pose_pred[:, :3]))
        #A=pose_pred[:, :3]
        #B=A.T
        #print(np.dot(B,A))
    ...
    def add_metric10(self, pose_pred, pose_targets, icp=False, syn=False, percentage=0.1):   #模型直径的10%
        # 输出的R,T
        #print("pose_pred",pose_pred[:, :3])
        #print("pose_targets",pose_targets[:, :3])
        #print(np.linalg.det(pose_targets[:, :3]))
        ...
        #B=A.T
        #print(np.dot(B,A))
        #print(np.multiply(pose_pred[:, :3] , pose_pred[:, :3].T))  #验证预测的pose是否为单位阵
        ...
        if icp:
            self.icp_add10.append(mean_dist < diameter)
        else:
            self.add10.append(mean_dist < diameter)
            
    def add_metric5(self, pose_pred, pose_targets, icp=False, syn=False, percentage=0.05):   #模型直径的5%
        diameter = self.diameter * percentage
        model_pred = np.dot(self.model, pose_pred[:, :3].T) + pose_pred[:, 3]
        model_targets = np.dot(self.model, pose_targets[:, :3].T) + pose_targets[:, 3]
        ...
        if icp:
            self.icp_add5.append(mean_dist < diameter)
        else:
            self.add5.append(mean_dist < diameter)
            
    def add_metric2(self, pose_pred, pose_targets, icp=False, syn=False, percentage=0.02):   #模型直径的2%
        diameter = self.diameter * percentage
        model_pred = np.dot(self.model, pose_pred[:, :3].T) + pose_pred[:, 3]
        model_targets = np.dot(self.model, pose_targets[:, :3].T) + pose_targets[:, 3]
        ...
        if icp:
            self.icp_add2.append(mean_dist < diameter)
        else:
    ...
    def cm_degree_2_metric(self, pose_pred, pose_targets, icp=False):
        translation_distance = np.linalg.norm(pose_pred[:, 3] - pose_targets[:, 3]) * 100
        rotation_diff = np.dot(pose_pred[:, :3], pose_targets[:, :3].T)
        trace = np.trace(rotation_diff)
        trace = trace if trace <= 3 else 3
        angular_distance = np.rad2deg(np.arccos((trace - 1.) / 2.))
        if icp:
            self.icp_cmd2.append(translation_distance < 2 and angular_distance < 2)
        else:
            self.cmd2.append(translation_distance < 2 and angular_distance < 2)
    ...
    def uncertainty_pnp(self, kpt_3d, kpt_2d, var, K):  # 2D-3D对应，使用pnp计算物体位姿
        cov_invs = []
    ...
    def evaluate(self, output, batch):
        ...
        if cfg.test.icp:
            ...
            if cfg.cls_type in ['eggbox', 'glue']:
                self.add_metric10(pose_pred_icp, pose_gt, syn=True, icp=True)
                self.add_metric5(pose_pred_icp, pose_gt, syn=True, icp=True)
                self.add_metric2(pose_pred_icp, pose_gt, syn=True, icp=True)
            else:
                self.add_metric10(pose_pred_icp, pose_gt, icp=True)
                self.add_metric5(pose_pred_icp, pose_gt, icp=True)
                self.add_metric2(pose_pred_icp, pose_gt, icp=True)
            self.projection_2d(pose_pred_icp, pose_gt, K, icp=True)
            self.cm_degree_5_metric(pose_pred_icp, pose_gt, icp=True)
            self.cm_degree_2_metric(pose_pred_icp, pose_gt, icp=True)

        if cfg.cls_type in ['eggbox', 'glue']:
            self.add_metric10(pose_pred, pose_gt, syn=True)
            self.add_metric5(pose_pred, pose_gt, syn=True)
            self.add_metric2(pose_pred, pose_gt, syn=True)
        else:
            self.add_metric10(pose_pred, pose_gt)
            self.add_metric5(pose_pred, pose_gt)
            self.add_metric2(pose_pred, pose_gt)
        self.projection_2d(pose_pred, pose_gt, K)
        self.cm_degree_5_metric(pose_pred, pose_gt)
        self.cm_degree_2_metric(pose_pred, pose_gt)
        self.mask_iou(output, batch)

    def summarize(self):
        proj2d = np.mean(self.proj2d)
        add10 = np.mean(self.add10)
        add5 = np.mean(self.add5)
        add2 = np.mean(self.add2)
        cmd5 = np.mean(self.cmd5)
        cmd2 = np.mean(self.cmd2)
        ap = np.mean(self.mask_ap)
        
        print('2d projections metric: {}'.format(proj2d))
        print('ADD-10% metric: {}'.format(add10))
        print('ADD-5% metric: {}'.format(add5))
        print('ADD-2% metric: {}'.format(add2))
        print('5 cm 5° metric: {}'.format(cmd5))
        print('2 cm 2° metric: {}'.format(cmd2))
        print('mask ap70: {}'.format(ap))
        if cfg.test.icp:
            print('2d projections metric after icp: {}'.format(np.mean(self.icp_proj2d)))
            print('ADD-10% metric after icp: {}'.format(np.mean(self.icp_add10)))
            print('ADD-5% metric after icp: {}'.format(np.mean(self.icp_add5)))
            print('ADD-2% metric after icp: {}'.format(np.mean(self.icp_add2)))
            print('5 cm 5° metric after icp: {}'.format(np.mean(self.icp_cmd5)))
            print('2 cm 2° metric after icp: {}'.format(np.mean(self.icp_cmd2)))
        self.proj2d = []
        self.add10 = []
        self.add5 = []
        self.add2 = []
        ...
        self.icp_cmd5 = []
        self.icp_cmd2 = []
        return {'proj2d': proj2d, 'add-10%': add10,'add-5%': add5,'add-2%': add2, 'cmd5': cmd5, 'cmd2': cmd2, 'ap': ap}
